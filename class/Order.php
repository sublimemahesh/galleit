...
class Order {
    ...
    public function getOrdersByDateRange($from, $to, $status) {
        ...
        $query = "SELECT * FROM `orders` WHERE `delivery_status`='" . $status . "' AND `status`='1' AND `payment_status_code`='2' AND (`ordered_at` BETWEEN '" . $from . "' AND '" . $to . "' OR `ordered_at` LIKE '%" . $to . "%')";
        $db = new Database();
        $result = $db->readQuery($query);
        $array_res = array();
    ...
    public function getUnpaidOrdersByDateRange($from, $to) {
        ...
        $query = "SELECT * FROM `orders` WHERE `status`='0' AND `payment_status_code` <> '2' AND (`ordered_at` BETWEEN '" . $from . "' AND '" . $to . "' OR `ordered_at` LIKE '%" . $to . "%') ";
        $db = new Database();
        $result = $db->readQuery($query);
        $array_res = array();
